# for each bootstrap iteration: match words, use each word's covariate vector as predictor (y=1=success, y=0=failure) 
# then predict success/failure using the mean predictors over timesteps s-k through s
library('MatchIt')
library(data.table)
library('optmatch')
library('caret')
...
combine_covariate_data <- function(k, matches, covariates, covariate_names, verbose=TRUE){
	...
	# print('got final covariate vals matches')		
	# print(covariate_vals_matches)
	# convert to data frame for proper representation in list form
	covariate_vals_matches <- as.data.frame(covariate_vals_matches)
        return(covariate_vals_matches)
    })
    # print('got all covariates raw')
    # print(head(all_covariates))
    # print(class(all_covariates))
...
for(f in 1:feature_set_count){
    ...
    print(paste('got accuracy results with shape ', paste(dim(accuracy_results_relevant), collapse=','), sep=''))
...
accuracy_out_file <- paste(out_dir, paste('bootstrap_matched_success_failure_k', k, '_non_differenced_mean_accuracy_', folds, 'fold.tsv', sep=''), sep='/')
coefficient_out_file <- paste(out_dir, paste('bootstrap_matched_success_failure_k', k, '_non_differenced_mean_coefficients_', folds, 'fold.tsv', sep=''), sep='/')
